...
def validar_requisitos_literais(matched, curriculum_text):
    matched_filtrado = []
    curriculum_lower = curriculum_text.lower()
    for requisito in matched:
        # Verifica se requisito aparece literalmente (ignorando caixa)
        if requisito.lower() in curriculum_lower:
            matched_filtrado.append(requisito)
    return matched_filtrado
...
{{
    ...
    "observation": "Resumo em português ressaltando pontos fortes e fracos, graduação explícita e cálculo de experiência baseado somente no texto fornecido"
}}
...
"""
    ...
    try:
        ...
        # Validar matched_requirements para conter somente itens literais no currículo
        content['matched_requirements'] = validar_requisitos_literais(content['matched_requirements'], resume_text)

        # Recalcular score baseado na lista filtrada
        total_requisitos = len(content['matched_requirements']) + len(content['missing_requirements'])
        if total_requisitos > 0:
            content['score'] = int(len(content['matched_requirements']) / total_requisitos * 100)
        else:
            content['score'] = 0  # Evita divisão por zero se ambos vazios
